...
export class Client {
   ...
   * @param {RegisterOptions} options Options for the registration process
   */
  public async register(token: string, options?: RegisterOptions): PromiseResult<TokenResponse>;
  ...
  public async register(
    token: string,
    optionsOrCredentialNickname?: RegisterOptions | string
  ): PromiseResult<TokenResponse> {
    try {
      const options =
        typeof optionsOrCredentialNickname === 'string'
          ? { credentialNickname: optionsOrCredentialNickname }
          : optionsOrCredentialNickname;
      ...
      return await this.registerComplete(credential, registration.session, options);
  ...
  private async registerBegin(token: string): PromiseResult<RegisterBeginResponse> {
    const response = await fetch(combineUrl(this.config.apiUrl, 'register/begin'), {
      method: 'POST',
  ...
  private async registerComplete(
    ...
    const response = await fetch(combineUrl(this.config.apiUrl, 'register/complete'), {
      method: 'POST',
      headers: this.createHeaders(),
      body: JSON.stringify({
        session: session,
        ...
        nickname: options?.credentialNickname,
        authenticatorDisplayName: options?.authenticatorDisplayName,
        RPID: this.config.rpid,
        Origin: this.config.origin
      })
    });
